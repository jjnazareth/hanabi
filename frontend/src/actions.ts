...
import {
  ...
  RemoveCardFromHand, AddCardToHand
} from './reducers/room/room.actions.type'

import {
  GameActionNames, SetCurrentTurnIdx, SetNextTurnIdx, SetDealerIdx, InitDeck,
...
export const discard = (card: Card, player: Player, deck: Card[]) =>
  (dispatch: Dispatch<RemoveCardFromHand | AddCardToDiscardPile
    | RemoveCardFromDeck | AddCardToHand>) => {
    // discard
    dispatch({
...
export const build = (card: Card, player: Player, deck: Card[]) =>
  (dispatch: Dispatch<RemoveCardFromHand | AddCardToBuildPile
    | RemoveCardFromDeck | AddCardToHand>) => {
    // build
    dispatch({
      type: RoomActionNames.REMOVE_CARD_FROM_HAND,
